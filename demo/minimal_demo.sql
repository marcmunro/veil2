...
$$
begin
  -- Deleting the friend request, means that the frienship gets
...
values (5, 1, 'viewer', false, true, 'can view all appropriate data'),
       (6, 1, 'poster', false, true, 'can make posts'),
       (7, 1, 'user', true, false, 'can use the system'),
       (8, 1, 'admin', true, false, 'can administer the system');
...
insert
  into users
       (accessor_id, username, password)
values ('18', 'Pippa', 'passwd_pippa');
select * from users;

select veil2.init();
delete from users where accessor_id = 18;
select * from veil2.accessors;
select * from veil2.authentication_details;
